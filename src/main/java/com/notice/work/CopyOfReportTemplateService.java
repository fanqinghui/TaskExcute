...
@Service
public class CopyOfReportTemplateService {
	private static final Log log = LogFactory.getLog(CopyOfReportTemplateService.class);
    @Resource
	...
	public Map<String,Object> getTemplate(Long taskId) throws Exception{
			...
			//String sql ="select  t1.FILE_NAME as name ,t1.SUFFIX_CODE as code ,t1.FILE_PATH  as url,t1.BILL_ATTACHMENT_CODE as type,t2.is_send_email as sendEmail " +
					 // "  from sys_file_relations t1,sys_task_all t2 where  t1.BILL_ID = t2.TASK_ID and t1.disabled='0' and t1.bill_id="+taskId;
		//	List<Map<String,Object>> list = (List<Map<String,Object>>)this.jdbcTemplate.queryForList(sql);
			List<Map<String,Object>> list =taskHandler.getFileRelations(taskId);
			if(list.size()>0){
...
//				resultMap.put("name", ParseUtil.getString(map.get("url"))+"/"+ParseUtil.getString(map.get("name")));
			...
			//String paramSql ="select PARAM_NAME as key,PARAM_VALUE as value from sys_param_all t where (DISABLE_DATE is null or DISABLE_DATE>sysdate) and t.task_id="+taskId;
			//获取任务关联参数信息供模板解析调用
			List<Map<String,Object>> paramList =taskHandler.getParamAllByTaskId(taskId); 
					//(List<Map<String,Object>>)this.jdbcTemplate.queryForList(paramSql);
	...
	public String getSqlByCode(String code) throws Exception {
					...
					List<SysDataSource> list = taskHandler.getSqlByCode(code);
					//("select PS_DEFINING from SYS_DATA_SOURCE where CODE ='"+code+"'");
					if(list.size()>0)
					{
						SysDataSource dataSource = list.get(0);
						String clob =dataSource.getPS_DEFINING();
						if(clob!=null)
						{
							sql =clob.toString();	
						}	
    ...
    public String executeSql(String sql,String code) throws Exception {
		...
		try {
			...
			result=taskHandler.executeSql(sql);
			return result;
		} catch (Exception e) {
